...
void cmdInput_sm_main(eventtype const *event)
{
	...
	switch (cmdInput_sm.statem.state)
	{
		...
		break;
		case FAILSAFESET_REQ:
			switch(event->sig)
			{
				case ENTRY_SIG:
					/* Entry call code */
					cmdInput_sm.cnt = FAILSAFESET_REQ_TIME;
				break;
				case TSLOT_80ms:
					/* Button should be kept pushed for the FAILSAFESET Period */
					if(PUSHBUTTOM_ON)
					{
						if(cmdInput_sm.cnt > 0)
						{
						...
						else
						{
							...
							loc_signal_p->event.sig = PBUTTOM_FSSETREQ_SIG;
							/* Send Signal */
							sendq_Sevent((eventtype *) loc_signal_p);
			...
			switch(event->sig)
			{
				...
				case TSLOT_80ms:
					if(cmdInput_sm.cnt != 0)
					{
						...
						/* otherwise we reset waitting time                 */
						if(PUSHBUTTOM_ON)
						{
							cmdInput_sm.cnt = STANDBY_TIME_PUSH;
						}
					...
					{
						cmdInput_sm.statem.state = FAILSAFESET_REQ;
						/* Exit State Signal */
						cmdInput_sm_main(EXIT_STATE);
					}
				break;
